...
def webhook():
    ...
    if req['queryResult']['intent']['displayName'] == 'ask_math':
        if req['queryResult']['parameters']['animal'] == 'cat':
        ...
        elif req['queryResult']['parameters']['animal'] == 'dog':
            ...
            if req['queryResult']['parameters']['operator'] == "+" :
                ...
                if number <= 20:
                    ans = "汪!" * int(number)
                    return jsonify({"fulfillmentText":"{}。旺了{}次，我要罐罐~".format(ans, int(number))})
                elif number >20:
                    return jsonify({"fulfillmentText":"汪!汪!汪! 我要汪{}次，很累ㄟ。".format(int(number))})
                #else:
                #    return jsonify({"fulfillmentText":"你不找咪咪嗎"})
